...
makeCircle() - makes a Box2D body in the shape of a circle - see parameters
makeTriangle() - makes a Box2D body in the shape of a triangle - see parameters
		...
		function updateMap() {
			for (var i=0; i<mappings.length; i++) {
				...
				if (!scroll) {
					...
					if (that.followObj && that.followObj==zimObj && zimObj.followDampX) frame.stage.x = frame.stage.scaleX*zimObj.followDampX.convert(zimObj.offsetDampX.convert(zimObj.followOffsetX[zimObj.controlX])-zimObj.x);
					if (that.followObj && that.followObj==zimObj && zimObj.followDampY) frame.stage.y = frame.stage.scaleY*zimObj.followDampY.convert(zimObj.offsetDampY.convert(zimObj.followOffsetY[zimObj.controlY])-zimObj.y);

					if (zimObj.frameBorderLock) {
						if (frame.stage.x < frame.stage.scaleX*borders.x && ((zimObj.frameBorderOriginal && that.borderRight.m_fixtureCount == 0) || that.borderRight.m_fixtureCount == 1)) frame.stage.x = frame.stage.scaleX*borders.x;
						if (frame.stage.y < frame.stage.scaleY*borders.y && ((zimObj.frameBorderOriginal && that.borderBottom.m_fixtureCount == 0) || that.borderBottom.m_fixtureCount == 1)) frame.stage.y = frame.stage.scaleY*borders.y;
						if (frame.stage.x > frame.stage.scaleX*(borders.x + borders.width - frame.stage.width) && ((zimObj.frameBorderOriginal && that.borderLeft.m_fixtureCount == 0) || that.borderLeft.m_fixtureCount == 1)) frame.stage.x = frame.stage.scaleX*(borders.x + borders.width - frame.stage.width);
						if (frame.stage.y > frame.stage.scaleY*(borders.y + borders.height - frame.stage.height) && ((zimObj.frameBorderOriginal && that.borderTop.m_fixtureCount == 0) || that.borderTop.m_fixtureCount == 1)) frame.stage.y = frame.stage.scaleY*(borders.y + borders.height - frame.stage.height);
					}
				}
